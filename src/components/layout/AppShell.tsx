...
export const AppShell = ({ children }: AppShellProps) => {
  ...
  const navigation = {
    ...
    staff: [
      { name: "Dashboard", href: "/staff", icon: BarChart },
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="container mx-auto px-4 flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-4">
            {currentNavigation.map((item) => (
              <Link
                ...
                to={item.href}
                className={cn(
                  "text-sm font-medium transition-colors hover:text-crocus-500",
          ...
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" size="sm" onClick={cycleUserRole} className="hidden md:flex">
      ...
      <div
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-3/4 max-w-xs bg-white transform transition-transform duration-300 ease-in-out md:hidden",
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="p-4 border-b">
          <div className="flex items-center justify-between">
            <Link to="/" className="font-bold text-xl text-crocus-500">
              CROCUS
            </Link>
        ...
        <nav className="p-4 space-y-2">
          ...
          <div className="pt-4 mt-4 border-t">
            <Button variant="outline" size="sm" onClick={cycleUserRole} className="w-full">
              Switch to: {userRole === "guest" ? "User" : userRole === "user" ? "Staff" : userRole === "staff" ? "Admin" : "Guest"}
            </Button>
          </div>
        </nav>
      </div>
